...
const ILikedIt = ({ data }) => {
  ...
  const likeHandler = async () => {
    discuss = await discussions.find(dis => dis.id === data.id)
    discuss.iLikedIt = await discuss.iLikedIt ? false : true
  ...
  return (
    ...
    onClick={likeHandler}
      className={`${
        likedIt
          ? "bg-[#235ee7] text-white"
          : "bg-[#f4f5fa] text-[#2e3035]"
